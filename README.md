...
<body class="min-h-full bg-gradient-to-br from-slate-50 to-indigo-50 text-slate-900">
  ...
  const db = {
    // {id,title,description,category,date_time,venue,max,price,organiser_email,is_published,announcement,participants:[{id,user_email,name,ticket,qty,paid,amount,created_at}]}
    events: load(DB_KEY, []),
  }
    ...
    view.innerHTML = `
      <div class="max-w-md mx-auto card">
        ...
        <div class="mt-4 space-y-3">
          ...
          <p class="text-xs text-slate-600 text-center">Not registered? <a class="underline" href="#/register">Register as a Student</a></p>
        </div>
      </div>`;
    lucide.createIcons();
  })
  ...
  route('#/register', ()=>{
    view.innerHTML = `
      <div class="max-w-md mx-auto card">
        <h2 class="text-2xl font-black">Create your Student Account</h2>
        <p class="text-slate-600 text-sm mt-1">Join <b>Eventx</b> to attend events.</p>
        <div class="mt-4 space-y-3">
          ${field('su_name','Name')}
          ${field('su_email','Email','email')}
          ${field('su_password','Password (6+ chars)','password')}
          <button class="btn btn-primary w-full" onclick="onRegister('student')">Create Account</button>
          <p class="text-xs text-slate-600 text-center">Already have an account? <a class="underline" href="#/login">Login here</a></p>
          <p class="text-xs text-slate-600 text-center">Are you an organiser? <a class="underline" href="#/register/organiser">Register here</a></p>
        </div>
      </div>`;
    lucide.createIcons();
  })
  ...
  route('#/register/organiser', ()=>{
    view.innerHTML = `
      <div class="max-w-md mx-auto card">
        <h2 class="text-2xl font-black">Create your Organiser Account</h2>
        <p class="text-slate-600 text-sm mt-1">Join <b>Eventx</b> to host and manage events.</p>
        <div class="mt-4 space-y-3">
          ${field('su_name','Organiser Name')}
          ${field('su_email','Email','email')}
          ${field('su_password','Password (6+ chars)','password')}
          <button class="btn btn-primary w-full" onclick="onRegister('organiser')">Create Account</button>
          <p class="text-xs text-slate-600 text-center">Already have an account? <a class="underline" href="#/login">Login here</a></p>
          <p class="text-xs text-slate-600 text-center">Are you a student? <a class="underline" href="#/register">Register here</a></p>
        </div>
      </div>`;
    lucide.createIcons();
  })
  ...
  route('#/event/:id', (params)=>{
    ...
    view.innerHTML = `
      <div class="grid lg:grid-cols-3 gap-6">
        ${e.announcement ? `
          <div class="lg:col-span-3 card bg-indigo-100/80 border-indigo-200">
            <h4 class="font-bold text-indigo-800 flex items-center gap-2"><i data-lucide="megaphone" class="w-4 h-4"></i>Announcement</h4>
            <p class="text-indigo-900 mt-1">${e.announcement}</p>
          </div>
        ` : ''}
        ...
        <aside class="card">
          <h4 class="font-bold text-lg">Registration Details</h4>
    ...
    view.innerHTML = `
      <div class="max-w-3xl mx-auto card">
        ...
        <div class="mt-4 grid sm:grid-cols-2 gap-4">
          ...
          ${field('st_qty','Quantity','number',`min=1 value=1 oninput="updateTotal(${(e.price||0)})"`)}
          <div>
            <label class="block text-sm font-medium">Total Amount (₹)</label>
            <input id="st_amount" class="input mt-1 bg-slate-100" readonly value="${(e.price||0).toFixed(2)}">
          </div>
        </div>
        <div class="mt-4">
    ...
    return `<tr class="border-t">
      ...
      <td class="text-right">
        <div class="inline-flex gap-1 flex-wrap justify-end">
          ...
          <button class="btn btn-ghost" onclick="openAnnounceModal('${e.id}')"><i data-lucide=megaphone class=w-4 h-4></i>Announce</button>
          <button class="btn btn-ghost" onclick="openEventModal('${e.id}')"><i data-lucide=pencil class=w-4 h-4></i>Edit</button>
          <button class="btn btn-ghost" onclick="togglePublish('${e.id}')">${e.is_published?'Unpublish':'Publish'}</button>
  ...
  window.openEventModal = function(id){
    const isEdit = !!id;
    const e = isEdit ? db.events.find(x=>x.id===id) : { title:'', description:'', category:'', date_time:new Date().toISOString().slice(0,16), venue:'', max:0, price:0, is_published:false };
  ...
  window.openAnnounceModal = function(id){
    ...
    dlg.innerHTML = `
      <form method="dialog" class="card">
        <h3 class="text-xl font-bold">Announcement — ${e.title}</h3>
        <p class="text-sm text-slate-600 mt-1">This message will be shown to students on the event details page.</p>
        <textarea id="announcement_text" class="input mt-3" rows="4" placeholder="Enter announcement...">${e.announcement||''}</textarea>
        <div class="mt-4 flex items-center justify-end gap-2">
          <button class="btn btn-ghost">Cancel</button>
          <button class="btn btn-primary" onclick="saveAnnouncement('${id}');return false;">Publish</button>
        </div>
      </form>`;
  ...
  window.saveEvent = function(id){
    ...
    if(id){
      ...
      Object.assign(e,{title,category,date_time,venue,max,price,description,updated_at:new Date().toISOString()});
    } else {
      db.events.push({ id:uid(), title, category, date_time, venue, max, price, description, organiser_email: session.email, is_published:false, announcement: '', participants:[], created_at:new Date().toISOString(), updated_at:new Date().toISOString() });
    }
    persist();
    $('#eventModal').close();
    setToast('Saved');
  ...
  function matchRoute(){
    ...
    for(const pattern in routes){
      ...
      if(pattern.includes('/:')){
        ...
        if(hash.startsWith(base)){
          const segs = hash.slice(base.length+1).split('/');
          if(segs.length === rest.length){
            rest.forEach((part, i) => params[part] = segs[i]);
    ...
    // Fallback for parameterized routes like #/register/:id
    const hashParts = hash.split('/');
    if (hashParts[0] === '#register' && hashParts.length > 1) {
      return { handler: routes['#/register/:id'], params: {id: hashParts[1]} };
    }
